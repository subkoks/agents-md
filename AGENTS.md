...
| Local CI mirror (lint+check+test) | `make ci` |
...
- No language runtime dependencies (no npm/pip/cargo install needed). Only system tools: `bash`, `shellcheck`, `markdownlint-cli2`, `actionlint`, `pre-commit`, `bats` (for `make test`).
- Tests live in `tests/*.bats` and are hermetic: each builds a throwaway repo copy under `$BATS_TEST_TMPDIR` and never touches the working tree or real `$HOME`.
