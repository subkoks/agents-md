...
| Rule updates            | `src/gotcha.md`                     | `docs/extending-rules.md`       | source rule diff + changelog entry    |
...
| Rule artifact packaging | `docs/editor-alignment-strategy.md` | `scripts/check-local-drift.sh`  | drift report + artifact build actions |
...
- If route is ambiguous, default to repository source first (`src/gotcha.md` for behavior, `docs/*` for reference).
